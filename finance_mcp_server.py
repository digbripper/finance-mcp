...
def export_voter_csv(
    ...
    county:                  str = "",
    party:                   str = "",
    election_years:          list | None = None,
    voted_in_all_years:      bool = True,
    min_voter_score:         int  = 0,
    min_age:                 int  = 0,
    max_age:                 int  = 0,
    limit:                   int  = 0,
) -> dict:
    """
    Query the SQLite voter database with any combination of filters and
    write the results to a downloadable CSV file on the Railway server.

    Filters
    -------
    congressional_district  "13"  — NY-13
    state_senate_district   "27"  — NY State Senate district
    assembly_district       "72"  — NY Assembly district
    council_district        "44"  — NYC Council district
    zip_code                "11215"
    county                  "brooklyn" / "manhattan" / "queens" / "bronx" / "staten island"
    party                   "DEM" / "REP" / "WFP" / "DEM,WFP" (comma-sep for multiple)
    election_years          [2016, 2020, 2024]  — years to filter on (general elections)
    voted_in_all_years      True  → must appear in ALL listed years
                            False → must appear in ANY of the listed years
    min_voter_score         Minimum composite voter score (GE + primary votes)
    min_age / max_age       Age range in years (uses DOB field)
    limit                   Cap results at N rows (0 = no cap)
    ...
    -------
    {
      "status":        "ready",
      "rows":          67234,
      "filename":      "voter_export_abc123.csv",
      "download_url":  "/download/voter_export_abc123.csv",
      "columns":       [...],
      "preview":       [first 3 rows as dicts],
      "filters_used":  {...},
      "note":          "..."
    }
    """
    ...
    if _voter_db_conn is None:
        return {"status": "error", "message": "Voter DB not loaded — call inspect_voter_db first to check status"}

    # ── Build WHERE clause ────────────────────────────────────────────────────
    conditions: list[str] = ["status = 'A'"]   # active voters only
    params:     list      = []

    def _like_year(col: str, year: int) -> str:
        """Safe exact-year match in a comma-separated year string."""
        return (f"({col} = ? OR {col} LIKE ? OR {col} LIKE ? OR {col} LIKE ?)")

    def _like_year_params(year: int) -> list:
        y = str(year)
        return [y, f"{y},%", f"%,{y},%", f"%,{y}"]

    # Congressional / state / city districts
    if congressional_district.strip():
        conditions.append("cd = ?")
        params.append(congressional_district.strip())

    if state_senate_district.strip():
        conditions.append("sd = ?")
        params.append(state_senate_district.strip())

    if assembly_district.strip():
        conditions.append("ad = ?")
        params.append(assembly_district.strip())

    if council_district.strip():
        conditions.append("council_district = ?")
    ...
    if county.strip():
        # Normalise to title-case county name (Staten Island, Brooklyn, etc.)
        _COUNTY_MAP = {
        ...
        cn = _COUNTY_MAP.get(county.strip().lower(), county.strip().title())
        conditions.append("county_name = ?")
        params.append(cn)

    # Party — supports comma-separated multi-party ("DEM,WFP")
    if party.strip():
    ...
    # Election year filtering on ge_years (comma-separated year list in the DB)
    if election_years:
        years = [int(y) for y in election_years]
        if voted_in_all_years:
            # Voter must appear in EVERY listed year
            for yr in years:
                conditions.append(_like_year("ge_years", yr))
                params.extend(_like_year_params(yr))
        else:
            # Voter must appear in AT LEAST ONE listed year
            year_clauses = [_like_year("ge_years", yr) for yr in years]
            conditions.append(f"({' OR '.join(year_clauses)})")
            for yr in years:
                params.extend(_like_year_params(yr))

    # Voter score
    if min_voter_score > 0:
        conditions.append("voter_score >= ?")
        params.append(min_voter_score)

    # Age range (DOB stored as YYYYMMDD text)
    current_year = _dt.datetime.now().year
    if min_age > 0:
        max_birth_year = current_year - min_age
        conditions.append("CAST(SUBSTR(dob,1,4) AS INTEGER) <= ?")
    ...
    # ── Count first ──────────────────────────────────────────────────────────
    try:
        count_row = _voter_db_conn.execute(
            f"SELECT COUNT(*) FROM voters {where_sql}", params
        ).fetchone()
    ...
    if total_rows == 0:
        return {
            ...
            "filters_used": {
                ...
                "voted_in_all_years":     voted_in_all_years,
            },
        }
    ...
    select_sql = f"""
        SELECT
            lastname, firstname, middlename, dob, party,
            address, city, zip,
            cd          AS congressional_district,
            sd          AS state_senate_district,
            ad          AS assembly_district,
            council_district,
            county_name, regdate,
        ...
        FROM voters
        {where_sql}
        ORDER BY county_name, lastname, firstname
        {limit_sql}
    """

    try:
        rows = _voter_db_conn.execute(select_sql, params).fetchall()
    except Exception as exc:
        return {"status": "error", "message": f"Select query failed: {exc}"}
    ...
    columns = [
        "last_name", "first_name", "middle_name", "date_of_birth", "party",
        "address", "city", "zip",
        "congressional_district", "state_senate_district", "assembly_district",
        "council_district", "county", "registration_date",
        "ge_votes", "primary_votes", "voter_score",
        "ge_years", "primary_years",
    ]
    ...
    try:
        ...
        return {"status": "error", "message": f"CSV write failed: {exc}"}

    # Preview (first 3 rows as dicts)
    preview = [dict(zip(columns, list(r))) for r in rows[:3]]

    note = ""
    if limit and limit > 0 and total_rows > limit:
        note = (f"Result capped at {limit:,} rows. "
                f"Full dataset has {total_rows:,} matching voters. "
                f"Re-run with limit=0 to export all.")
    ...
    return {
        ...
        "filters_used": {
            ...
            "election_years":          election_years,
            "voted_in_all_years":      voted_in_all_years,
            "min_voter_score":         min_voter_score        or None,
